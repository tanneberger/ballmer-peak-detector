...
void write_special_cases(ofstream *file, posit<C_N, C_ES, CONTAINER> op0,
                         posit<C_N, C_ES, CONTAINER> op1) {
  cout << "Generating Special cases..." << endl;
  // 0+0
  op0.setbits(0);
  op1.setbits(0);

  *file << get_hex(op0) << " " << get_hex(op1) << " " << get_hex(op0 + op1)
        << endl;
  ...
  *file << get_hex(op0) << " " << get_hex(op1) << " " << get_hex(op0 + op1)
  ...
  // check special case (mantissa overflow -> rigthshift) wich made errors in
  // the past
  op0.setbits(66);
  op1.setbits(67);
...
void write_random_cases(ofstream *file, posit<C_N, C_ES, CONTAINER> op0,
                        posit<C_N, C_ES, CONTAINER> op1,
                        posit<C_N, C_ES, CONTAINER> op2, unsigned long num) {
  cout << "Generating " << num << " Random Testcases\n";

  for (uint64_t i = 0; i < num; i++) {
    unsigned long a = rand() % (1 << C_N);
    unsigned long b = rand() % (1 << C_N);
    unsigned long c = rand() % (1 << C_N);

    op0.setbits(a);
    op1.setbits(b);
    op2.setbits(c);

    *file << get_hex(op0) << " " << get_hex(op1) << " " << get_hex(op2) << " "
          << get_hex(op0 + op1) << endl;
  }
}

// CAREFULL!!!!! could take until heat death of the universe to test depending
// on bits
void write_all_cases(ofstream *file, posit<C_N, C_ES, CONTAINER> op0,
                     posit<C_N, C_ES, CONTAINER> op1) {
  cout << "Generating All possible Testvectors WARNING: Simulation can take a "
          "long time depending on Bitwidth\n";
...
int main(int argc, char *argv[]) {
  ...
  posit<C_N, C_ES, CONTAINER> op0, op1, op2, exp;

  if (select == 0) {
    write_special_cases(&file, op0, op1);
  } else if (select == 1) {
    // write_special_cases(&file, op0, op1);
    write_random_cases(&file, op0, op1, op2, rand_num);
  } else if (select == 2) {
