...
void write_random_cases(ofstream *file, posit<C_N, C_ES, CONTAINER> op0, posit<C_N, C_ES, CONTAINER> op1, posit<C_N, C_ES, CONTAINER> op2, unsigned long num){
  ...
  for(uint64_t i = 0; i < num; i++){
    ...
    *file << get_hex(op0) << " " << get_hex(op1) << " " << get_hex(op2) << " " << get_hex((op0 * op1)+op2) << endl;
  }
}

// CAREFULL!!!!! could take until heat death of the universe to test depending on bits
void write_all_cases(ofstream *file, posit<C_N, C_ES, CONTAINER> op0, posit<C_N, C_ES, CONTAINER> op1, posit<C_N, C_ES, CONTAINER> op2){
...
int main(int argc, char *argv[]) {
    ...
    if (select == 0) {
        ...
        write_all_cases(&file, op0, op1, op2);
    }
